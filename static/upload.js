...
function addSettingBtnListener() {
  let btns = document.getElementsByClassName("checkbox")
  for (let btn of btns) {
    btn.addEventListener("click", function () {
      for (let btn0 of document.getElementsByClassName("checkbox")) {
        btn0.className = "checkbox"
      }
      btn.className = "checkbox active"
    ...
    callback(
      ...
      (taskID) => {
        let timer = setInterval(() => {
          getTaskStatus(taskID, function (task) {
            ...
            if (task.status === 3) {
              progressBar.style.background = "#ffc940"
              progressBar.style.width = task.progress * 100 + "%";
              downloadButton.setAttribute("href",`/download?taskId=${taskID}`)
              fileinfo.innerHTML = `${task.name} 原大小:<b>${wellSize(task.size)}</b> 压缩后大小:<b>${wellSize(task.size2)}</b>`
